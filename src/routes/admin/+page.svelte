<script>
	...
	async function aprobarCurso(id) {
		await supabase.from('cursos').update({ aprobado: true }).eq('id', id);
		cursosPendientes = cursosPendientes.filter((c) => c.id !== id);
		cursosTodos = cursosTodos.map((c) => (c.id === id ? { ...c, aprobado: true } : c));
	}

	async function rechazarCurso(id) {
		await supabase.from('cursos').delete().eq('id', id);
		cursosPendientes = cursosPendientes.filter((c) => c.id !== id);
		cursosTodos = cursosTodos.filter((c) => c.id !== id);
	}
...
<div class="flex items-center justify-between mb-8">
	...
	<div class="flex gap-2">
		...
		>
			{agregandoCurso ? 'Cancelar' : '+ Agregar curso'}
		</button>
	</div>
</div>
...
<div class="flex gap-2 mb-6">
	<button
		onclick={() => (vistaAdmin = 'recursos')}
		class="px-4 py-2 rounded-xl text-sm font-medium transition-colors {vistaAdmin === 'recursos'
	...
	<button
		...
		class="px-4 py-2 rounded-xl text-sm font-medium transition-colors {vistaAdmin === 'cursos'
...
{:else}
	<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
		<div
			class="border border-gray-200 dark:border-gray-700 rounded-2xl p-4 sm:p-5 bg-white dark:bg-gray-800"
		>
			<p class="text-sm text-gray-400 dark:text-gray-500 mb-1">Cursos totales</p>
		...
		>
			<p class="text-sm text-green-600 dark:text-green-400 mb-1">Aprobados</p>
			<p class="text-3xl sm:text-4xl font-bold text-green-600 dark:text-green-400">
				{cursosTodos.filter((c) => c.aprobado).length}
			</p>
		</div>
	</div>
{/if}
...
{:else if agregando}
	<div
		class="border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-4 sm:p-6 mb-8 flex flex-col gap-4"
	>
...
{:else if agregandoCurso}
	<div
		class="border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20 rounded-2xl p-4 sm:p-6 mb-8 flex flex-col gap-4"
	>
		<h2 class="font-bold text-gray-900 dark:text-white">Agregar curso</h2>
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<input
				bind:value={tituloC}
				placeholder="Título"
		...
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
			<select
				...
				class="border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm bg-white dark:bg-gray-800 dark:text-white"
			>
				<option>Gratis</option><option>De pago</option><option>Freemium</option>
			</select>
			<select
				bind:value={nivelC}
				class="border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm bg-white dark:bg-gray-800 dark:text-white"
...
{:else}
	<div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
		...
		<input
			...
			class="w-full sm:flex-1 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-purple-400 dark:focus:border-purple-500 bg-white dark:bg-gray-800 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500"
		/>
	</div>
{/if}
...
{#if vistaAdmin === 'recursos'}
					...
					<div class="flex-1 min-w-0">
						<div class="flex items-center gap-2 mb-1 flex-wrap">
							...
							<span class="text-xs text-gray-400 dark:text-gray-500"
								>{recurso.tipo} · {recurso.nivel}</span
							>
							{#if recurso.aprobado}
								<span
	...
	{/if}
{:else if cursosMostrados().length === 0}
	<p class="text-gray-400 dark:text-gray-500 text-sm">No hay cursos.</p>
{:else}
	<div class="flex flex-col gap-3">
		{#each cursosMostrados() as curso (curso.id)}
			<div
				class="border border-gray-200 dark:border-gray-700 rounded-2xl p-5 flex items-start justify-between gap-4 bg-white dark:bg-gray-800"
			>
				<div class="flex-1 min-w-0">
					<div class="flex items-center gap-2 mb-1 flex-wrap">
						<span class="font-bold text-gray-900 dark:text-white">{curso.titulo}</span>
						<span class="text-xs text-gray-400 dark:text-gray-500"
							>{curso.precio} · {curso.nivel}</span
						>
						{#if curso.aprobado}
							<span
								class="text-xs bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 px-2 py-0.5 rounded-full"
					...
					{#if curso.instructor}
						<p class="text-sm text-purple-500 dark:text-purple-400 mb-1">
							Instructor: {curso.instructor}
						</p>
					{/if}
					<p class="text-sm text-gray-500 dark:text-gray-400 mb-1 truncate">{curso.descripcion}</p>
					<a
						href={curso.url}
						target="_blank"
						class="text-xs text-blue-500 dark:text-blue-400 hover:underline block truncate"
						>{curso.url}</a
					>
					{#if curso.autor_nombre}
						<p class="text-xs text-purple-500 dark:text-purple-400 mt-1">
							Enviado por: {curso.autor_nombre}
						</p>
					{/if}
				</div>
				<div class="flex flex-col gap-2 shrink-0">
					{#if !curso.aprobado}
						<div class="flex gap-1">
							<button
								onclick={() => aprobarCurso(curso.id)}
								class="bg-green-500 text-white px-2 py-1 rounded-lg text-xs hover:bg-green-600 transition-colors"
								>✓</button
							>
							<button
								onclick={() => rechazarCurso(curso.id)}
								class="bg-red-500 text-white px-2 py-1 rounded-lg text-xs hover:bg-red-600 transition-colors"
								>✕</button
							>
						</div>
	...
	{:else}
		<div class="flex flex-col gap-3">
			{#each mensajes.filter( (m) => (vistasMensajes === 'pendientes' ? !m.leido : m.leido) ) as m (m.id)}
				<div
					class="border border-gray-200 dark:border-gray-700 rounded-2xl p-5 bg-white dark:bg-gray-800"
				>
